...
class AdminController extends Controller
{
    ...
    public function waitress()
    {
        ...
        $total_waitress = DB::table('waitress')->where('status',0)->count();
        $fraction = $total_waitress % 3;
        $waitress = DB::table('waitress')->where('status',0)->get();
        $fraction_waitress = DB::table('waitress')->where('status',0)->latest()->get();
